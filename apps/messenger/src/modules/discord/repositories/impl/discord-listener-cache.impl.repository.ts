...
export class DiscordListenerCacheRepositoryImpl
  implements DiscordListenerCacheRepository
{
  ...
  public async findAllListeners(): Promise<DiscordListenerEntity[]> {
    const keys = await this.cacheService.getKeys(
      DiscordListenerCacheKeys.ALL_LISTENERS_KEY,
    );

    const cachedListeners = await CollectionUtils.asyncMap(
      keys,
      async (key) => {
        return await this.cacheService.get<DiscordListenerEntity>(key);
      },
    );
    return cachedListeners.filter(
      (val) => val != undefined,
